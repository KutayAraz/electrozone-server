...
export class BuyNowCartService {
    constructor(
        @InjectRepository(BuyNowSessionCart) private buyNowCartRepository: Repository<BuyNowSessionCart>,
    ...
    async createBuyNowCart(sessionId: string, productId: number, quantity: number): Promise<void> {
        ...
        return this.dataSource.transaction(async (transactionManager) => {
            ...
            buyNowCart.total = product.price * quantity;
    ...
    async getBuyNowCart(sessionId: string): Promise<CartResponse | null> {
        ...
        if (product.price !== buyNowCart.addedPrice) {
            ...
            buyNowCart.addedPrice = product.price;
            buyNowCart.total = product.price * buyNowCart.quantity;
            await this.buyNowCartRepository.save(buyNowCart);
        }
