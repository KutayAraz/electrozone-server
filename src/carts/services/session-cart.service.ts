...
export class SessionCartService {
  ...
  async addToSessionCart(
    ...
    return manager.transaction(async (transactionManager) => {
      const [sessionCart, product] = await Promise.all([
        this.cartUtilityService.findOrCreateSessionCart(
          sessionId,
          transactionManager,
        ),
  ...
  async mergeCarts(sessionId: string, userUuid: string): Promise<CartResponse> {
    ...
    return this.dataSource.transaction(async (transactionalEntityManager) => {
      ...
      const existingSessionCart = await transactionalEntityManager.findOne(
        SessionCart,
        {
          where: { sessionId },
      ...
      for (const sessionCartItem of existingSessionCart.cartItems) {
        ...
        const existingProduct = existingItemsMap.get(
        ...
        if (existingProduct) {
          ...
          const newQuantity =
            sessionCartItem.quantity + existingProduct.quantity;
